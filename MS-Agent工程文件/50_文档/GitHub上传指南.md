...
> 目标：把 面试准备助手（视觉版）从你的电脑传到 GitHub，让其他人（或你自己换电脑时）能下载使用。
...
├── MS-Agent工程文件\50_文档\新手安装使用指南.md          # 新手教程
├── MS-Agent工程文件\00_规范\                    # 设计文档与生成规范
├── MS-Agent工程文件\10_知识库/简历基准\                # ⚠ 你的个人简历 + 参与边界卡（涉及个人数据！）
├── MS-Agent工程文件\10_知识库/岗位画像\                # 各公司岗位画像模板
├── MS-Agent工程文件\10_知识库/面经实证\                # 面经记录（可能含个人信息）
├── MS-Agent工程文件\30_产出/面试材料\                # ⚠ 生成结果（含你的简历信息衍生内容）
└── MS-Agent工程文件\20_执行\
...
| `MS-Agent工程文件\20_执行/config.json` | ❌ 绝不传 | 里面有你的 API Key |
| `MS-Agent工程文件\20_执行/config.example.json` | ✅ 传 | 脱敏模板，别人照着填 |
| `MS-Agent工程文件/20_执行/node_modules` | ❌ 绝不传 | 首次启动由 runtime 自动解压，无需上传 |
| `MS-Agent工程文件/10_知识库/简历基准`（简历 PDF） | ❌ 建议不传 | 个人简历 = 个人隐私 |
| `MS-Agent工程文件/30_产出/面试材料` | ❌ 建议不传 | 生成内容基于你的简历 |
| `MS-Agent工程文件/10_知识库/面经实证` | ❌ 建议不传 | 含个人面试记录 |
...
4. 创建成功后，浏览器地址栏会显示 `https://github.com/你的用户名/MS-Agent` —— **把这个地址复制保存**，一会儿要用
...
cd /d D:\MS-Agent
```
> 注意：这次进的是 **MS-Agent**（项目根目录），不是 MS-Agent工程文件\20_执行。

### 5.2 检查"保护文件"（项目已自带，确认即可）

在**文件资源管理器**里确认两件事：

1. `D:\MS-Agent` 根目录**已有 `.gitignore`**（项目自带，无需手动创建）。它是 Git 的"黑名单"：列在里面的文件/文件夹——你的 `MS-Agent工程文件\20_执行\config.json`（含 Key）、简历、生成材料、node_modules——在 `git add` 时会被自动跳过，**不用手动记**。
   ...
   > ```
   > # 密钥与本地配置（绝不外传）
   > MS-Agent工程文件/20_执行/config.json
   > 
   > # 个人数据
   > MS-Agent工程文件/10_知识库/简历基准/
   > MS-Agent工程文件/30_产出/面试材料/
   > MS-Agent工程文件/10_知识库/面经实证/
   > 
   > # 依赖与系统文件
   > node_modules/
...
2. `MS-Agent工程文件\20_执行\config.json`（含你 Key 的那个）**留在本地不要动**；上传的是脱敏模板 `MS-Agent工程文件\20_执行\config.example.json`（Key 为空，它本来就传）。**不要**把 `config.example.json` 改名覆盖 `config.json`。

### 5.3 逐条执行命令

在终端 `D:\MS-Agent>` 下，**一条一条**输入下面的命令，每输入一条按一次回车：
...
```
git remote add origin https://github.com/你的用户名/MS-Agent.git
```
...
> To https://github.com/你的用户名/MS-Agent.git
...
1. 浏览器打开 `https://github.com/你的用户名/MS-Agent`
2. 确认能看到：`README.md`、`runtime` 文件夹、`MS-Agent工程文件` 文件夹、`启动面试准备助手.bat` 等
3. 点进 `runtime`，确认能看到 `node.zip` 和 `node_modules.zip.part1/2/3`（GitHub 单文件限制 100MB，已拆 3 片）
4. 点进 `MS-Agent工程文件\20_执行`，确认能看到 `config.example.json`、**看不到** `config.json` 和 `node_modules`
...
| 报错（英文） | 意思 | 解决 |
|--------------|------|------|
| `fatal: not a git repository` | 当前不在仓库里 | 确认终端路径是 `D:\MS-Agent>`，再执行 `git init` |
...
- [ ] 浏览器打开仓库，能看到代码
- [ ] MS-Agent工程文件\20_执行 里**没有** config.json、node_modules
- [ ] 根目录 `runtime` 里有 node.zip 和 node_modules.zip.part1/2/3；`runtime\node` / `runtime\node_modules.zip` 没被上传
- [ ] MS-Agent工程文件\10_知识库/简历基准 / MS-Agent工程文件\30_产出/面试材料 / MS-Agent工程文件\10_知识库/面经实证 没有上传
- [ ] 本地 `MS-Agent工程文件\20_执行/config.json` 还是原来的（Key 没被清掉）
