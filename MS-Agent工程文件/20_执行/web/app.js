// app.js — 面试准备助手（视觉版）面板前端逻辑（原生 JS，零构建）
(function () {
  "use strict";
  ...
  function renderKeyStatus() {
    ...
    if (verifiedUsable.length) {
      ...
      els.keyStatus.className = "key-status warn";
      els.keyStatus.innerHTML = "⚙ 已保存 " + providers.length + " 个 Key，但均<b>缺少文本生成能力</b>（cap 需含 text）。生成面试材料必须有文本模型，请把「🔑 第 1 步 → ④ 模型能力」改为「文本能力模型」后重新保存自检。";
    } else {
      els.keyStatus.className = "key-status info";
      els.keyStatus.innerHTML = "ℹ 尚未配置 API Key，目前无法生成材料。请按上方引导：「🔑 第 1 步」①选厂商 → ②选模型 → ③粘贴 Key →「④ 模型能力」选能力 → 点「💾 保存并自检」，状态条变绿即完成。";
    }
  }
