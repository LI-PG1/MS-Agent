...
(function () {
  ...
  if (location.protocol === "file:") {
    document.body.innerHTML =
      ...
      '<li><b>命令行启动</b>：打开终端，进入 <code>MS-Agent工程文件\\20_执行</code> 目录，执行 <code>npm install</code>（首次）后 <code>npm start</code>。</li>' +
      '</ol>' +
  ...
  const PRESETS = {
    zhipu:       { name: "zhipu", baseUrl: "https://open.bigmodel.cn/api/paas/v4/chat/completions", model: "glm-4-flash",
                   desc: "智谱 AI：模型 glm-4-flash（长期免费），文本生成够用；如需看图可选 glm-4v-flash。Base URL 已按官方填入。" },
    siliconflow: { name: "siliconflow", baseUrl: "https://api.siliconflow.cn/v1/chat/completions", model: "",
                   desc: "硅基流动：请在「模型广场」挑选模型后填入上方模型名（建议选带视觉能力的多模态模型，可覆盖看图 / OCR）。Base URL 已按官方填入。" },
    deepseek:    { name: "deepseek", baseUrl: "https://api.deepseek.com/v1/chat/completions", model: "deepseek-v4-flash",
                   desc: "DeepSeek：低成本文本方案（deepseek-v4-flash，价格低、Agent 能力强，新用户送额度）；看图可搭配智谱 glm-4v-flash（免费视觉）。若提示模型不存在请以平台「模型列表」为准。Base URL 已按官方填入。" },
  ...
  els.btnStart.addEventListener("click", async () => {
    ...
    // R9 必填项集中校验：岗位名称 / 简历上传 / 岗位 JD 文本（红字指出缺失项，未填字段红框提示）
    const missing = [];
    ...
    if (missing.length) {
      els.inputError.textContent = "必填项未填写：" + missing.join("、");
      if (!company) { els.inpCompany.classList.add("err-inp"); setTimeout(() => els.inpCompany.classList.remove("err-inp"), 2000); }
      if (!jdText && !jdUrl) { els.inpJd.classList.add("err-inp"); setTimeout(() => els.inpJd.classList.remove("err-inp"), 2000); }
  ...
  // ---------- 内置 OCR 引擎（P2：开箱即用，onnxruntime-node + PP-OCRv4） ----------
  async function loadOcrEngine() {
    ...
    try {
      ...
      const s = d.engine || {};
      if (s.ok) {
        el.innerHTML = '<span style="color:#059669;font-weight:600">✓ 就绪（可选增强兜底）</span><br><span class="mini">det ' + s.det + ' / rec ' + s.rec + ' / dict ' + s.dict + '；未配置视觉模型时自动回退本引擎提取图片文字</span>';
      } else {
        el.innerHTML = '<span style="color:#b45309;font-weight:600">✗ 模型缺失</span><br><span class="mini">缺少 ' +
          ...
          '（应位于 20_执行/assets/ocr/，随仓库发布；缺失时自动跳过）</span>';
      }
    } catch (e) {
      el.innerHTML = '<span style="color:#b45309;font-weight:600">✗ 引擎不可用</span><br><span class="mini">' + e.message + '（可选增强不可用时自动跳过）</span>';
    }
  }
