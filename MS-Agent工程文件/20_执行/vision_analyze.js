...
//
// 三级回退（P2 视觉解耦，v0.4.2）：
//   1. 内置 OCR 引擎（ocr_js.js：onnxruntime-node + PP-OCRv4 轻量模型，开箱即用、离线免费）
//   2. 本地 CLI OCR（cli_tools.js：rapidocr/paddleocr，本机装有 Python 的高级用户可选增强）
//   3. vision provider（多模态模型 API，多 provider 自动切换 + 失败 fallback 链）
// 效果：纯文本模型也能离线"看图读字"——图片 → 本地 OCR 提取文字 → 交给纯文本模型理解，
//       解耦对多模态 provider 的依赖；未配置视觉 key / 离线时本地 OCR 仍可用。
// 原则：三级全部为"可选增强"，任一环节缺失自动跳过，绝不阻塞主流程。
const fs = require("fs");
...
// 惰性加载：内置 OCR / CLI OCR 缺失时返回 null，不影响 vision provider 主链路
...
// ---------- 提取文字：vision provider 为主，本地 OCR 可选增强兜底（视觉版） ----------
async function extractText(imgPath) {
  ...
  // 兜底：本地 OCR（内置引擎 > CLI OCR，可选增强）
  const local = await extractTextLocal(imgPath);
  if (local.ok) return local;
  return { ok: false, error: errors.join("；") + "；本地 OCR 也失败：" + local.errors.join("；") };
}

// ---------- 完整视觉分析：vision provider 为主（视觉模型 / 多模态模型直接读图） ----------
// 本地 OCR 仅作 provider 缺失 / 全部失败时的可选增强兜底，不影响视觉模型主流程
async function analyzeImage(imgPath, prompt) {
  ...
  try {
    ...
    if (providers.length) {
      ...
      const local = await extractTextLocal(imgPath);
      if (local.ok) {
        return { ok: true, text: local.text, engine: local.engine, ms: local.ms, note: "视觉 provider 失败后本地 OCR 兜底；原错误：" + errors.join("；") };
      }
      return { ok: false, error: "全部视觉 provider 失败：" + errors.join("；") + "；本地 OCR 也失败：" + local.errors.join("；") };
    }
    // 未配置视觉 provider：本地 OCR 可选增强兜底
    const local = await extractTextLocal(imgPath);
    if (local.ok) {
      return { ok: true, text: local.text, engine: local.engine, ms: local.ms, note: "未配置视觉 provider，已用本地 OCR 提取文字" };
    }
    return { ok: false, error: "没有可用的 vision provider，且本地 OCR 失败：" + local.errors.join("；") };
  } catch (e) {
