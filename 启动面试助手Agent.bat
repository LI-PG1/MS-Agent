@echo off
chcp 65001 >nul
title 面试助手Agent（MS-Agent）一键启动
setlocal
set "LOCAL_VER=0.4.11"
rem 本文件位于发布包第一层，先进入工程文件目录
cd /d "%~dp0MS-Agent工程文件"
set PORT=8900
set "RUNTIME=%~dp0runtime"

echo ==================================================
echo   面试助手Agent（MS-Agent）一键启动器
...
set "REMOTE_VER="
for /f "delims=" %%v in ('powershell -NoProfile -Command "try{$r=Invoke-RestMethod -Uri 'https://api.github.com/repos/LI-PG1/MS-Agent/releases/latest' -TimeoutSec 5; Write-Output $r.tag_name}catch{Write-Output ''}"') do set "REMOTE_VER=%%v"
if defined REMOTE_VER if not "%REMOTE_VER%"=="v%LOCAL_VER%" (
    echo.
    echo [提示] 检测到新版本 %REMOTE_VER%（当前 v%LOCAL_VER%）。如需更新，请打开：
    echo        https://github.com/LI-PG1/MS-Agent/releases/latest
)
echo.
